...
public class MonthYearDateSlider extends DateSlider {
    ...
    protected Labeler yearLabeler  = new Labeler() {
                                       ...
                                       /**
                                        * add "val" year to the month object that contains "time" and returns the new
                                        * TimeObject
                                        */
                                       @Override
                                       public TimeObject add(final long time, final int val) {
                                        ...
                                        */
                                       @Override
                                       protected TimeObject timeObjectfromCalendar(final Calendar c) {
    ...
    protected Labeler monthLabeler = new Labeler() {
                                       ...
                                       /**
                                        * add "val" months to the month object that contains "time" and returns the new
                                        * TimeObject
                                        */
                                       @Override
                                       public TimeObject add(final long time, final int val) {
    ...
    public MonthYearDateSlider(final Context context, final OnDateSetListener l, final Calendar calendar) {
        super(context, l, calendar);
    }

    /**
     * Create the year and the monthscroller and feed them with their labelers and place them on the layout.
     */
    @Override
    public void onCreate(final Bundle savedInstanceState) {
        ...
        final LayoutParams lp = new LayoutParams(android.view.ViewGroup.LayoutParams.FILL_PARENT,
                android.view.ViewGroup.LayoutParams.WRAP_CONTENT);

        // create the year scroller and assign its labeler and add it to the layout
        final ScrollLayout mYearScroller = (ScrollLayout) inflater.inflate(R.layout.lib_dateslider_scroller, null);
        mYearScroller.setLabeler(this.yearLabeler, this.mTime.getTimeInMillis(), 200, 60);
        this.mLayout.addView(mYearScroller, 0, lp);
        this.mScrollerList.add(mYearScroller);

        // create the month scroller and assign its labeler and add it to the layout
        final ScrollLayout mMonthScroller = (ScrollLayout) inflater.inflate(R.layout.lib_dateslider_scroller, null);
        mMonthScroller.setLabeler(this.monthLabeler, this.mTime.getTimeInMillis(), 150, 60);
        this.mLayout.addView(mMonthScroller, 1, lp);
        this.mScrollerList.add(mMonthScroller);
    ...
    protected void setTitle() {
        if (this.mTitleText != null) {
            this.mTitleText.setText(getContext().getString(R.string.dateSliderTitle) + String.format(": %tB %tY", this.mTime, this.mTime));
        }
    }
